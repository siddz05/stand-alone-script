...
echo "Welcome, To The Git Clone Master Script. -- Validation Deployment Process!!"
# Variables
...
echo "======================================================="
echo
cloneDir="/home/ubuntu/product-validation-project/"
configDir="/product-validation-project/config/"
buildDir="/home/ubuntu/product-validation-project/product-validation-project"
basebackupDir="/home/ubuntu/product-validation-project-backup"
config=$1
branch_name=$2
user="ubuntu"
echo $config
...
if [ -z "$branch_name" ];
  then 
      ...
      echo "Pls use syntax : sh deploy_code_validation.sh beta/prod/stag master"
      exit 1
fi
#empty config Check
if [ -z "$config" ];
...
then
   echo "Config Is Correct -- deploy_code_validation.sh $config $branch_name"
else
   echo "Please, use sh master_deploye.sh beta master || sh master_deploye.sh stag master || sh master_deploye.sh prod master"
   exit 1
fi
cd ~
...
cd $cloneDir
echo "Cloning the branch repo dev_partial, $cloneDir"
#git pull origin master
git clone -b $branch_name --single-branch git@git.crowdanalytix.com:CAX-BE/product-validation-project.git
...
sudo cp /home/ubuntu/product-validation-project/product-validation-project/common/config/$config/*.py $configDir
...
cd $cloneDir"product-validation-project/"
git checkout -b validation
#Step 7 -- Stoping The Supervisor, Killing Ports and Restarting the Server!!
#Doing Migration
/opt/anaconda/bin/python /home/ubuntu/product-validation-project/product-validation-project/manage.py migrate
