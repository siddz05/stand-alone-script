...
echo "======================================================="
echo 
cloneDir="/home/ubuntu/pim-project-source/"
configDir="/pim-project/config/"
buildDir="/home/ubuntu/pim-project-source/pim-project"
basebackupDir="/home/ubuntu/pim-project-source-backup"
config=$1
user="ubuntu"
echo $config
...
if [ -z "$config" ];
  then 
      ...
      echo "Pls use syntax : sh pim_deploye.sh beta/prod/stag"
      exit 1
fi
#check, correct config
if [ "$config" = "beta" ] || [ "$config" = "stag" ] || [ "$config" = "prod" ];
then
   echo "Config Is Correct --pim_deploye.sh $config"
else
   echo "Please, use sh pim_deploye.sh beta || sh pim_deploye.sh stag || sh pim_deploye.sh prod"
   exit 1
fi
cd ~
...
git clone git@git.crowdanalytix.com:CAX-BE/pim-project.git
...
sudo cp /home/ubuntu/pim-project-source/pim-project/pim_common/config/$config/*.py $configDir
